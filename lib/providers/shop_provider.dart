...
class ShopProvider extends ChangeNotifier {
  static const _coinsKey = 'mq_coins';
  static const _ownedKey = 'mq_owned_items';
  static const _activeThemeKey = 'mq_active_theme';
  static const _activeBgKey = 'mq_active_background';
  static const _activeSkinKey = 'mq_active_skin';
  static const _lastDailyKey = 'mq_last_daily_reward';
  static const _favoriteRewardDateKey = 'mq_fav_reward_date';
  static const _favoriteRewardCountKey = 'mq_fav_reward_count';
  static const _shareRewardDateKey = 'mq_share_reward_date';
  static const _shareRewardCountKey = 'mq_share_reward_count';
  static const _processedPurchasesKey = 'mq_processed_purchases';
  static const _quoteFontKey = 'mq_quote_font';
  static const _notificationHourKey = 'mq_notification_hour';
  ...
  String get quoteFont => _quoteFont;
  ...
  bool get hasUnlimitedFavorites => _ownedItems.contains('feat_unlimited_favorites');
  bool get hasPremiumFonts => _ownedItems.contains('feat_premium_fonts');
  bool get hasCustomNotification => _ownedItems.contains('feat_custom_notification');
  bool get hasExportFavorites => _ownedItems.contains('feat_export_favorites');
  bool get hasWidgetStyles => _ownedItems.contains('feat_widget_styles');
  ...
  QuoteBackground get activeBackground => QuoteBackground.get(_activeBackgroundId);
  QuoteCardSkin get activeSkin => QuoteCardSkin.get(_activeSkinId);
  ...
  Future<void> _loadLocal() async {
    ...
    _quoteFont = await StorageService.instance.getString(_quoteFontKey) ?? 'outfit';
    _notificationHour = await StorageService.instance.getInt(_notificationHourKey) ?? 8;
  ...
  Future<void> _saveLocal() async {
    ...
    await StorageService.instance.saveString(_quoteFontKey, _quoteFont);
    await StorageService.instance.saveInt(_notificationHourKey, _notificationHour);
  ...
  Future<bool> rewardForFavorite() async {
    final today = _dateKey(DateTime.now());
    final savedDate = await StorageService.instance.getString(_favoriteRewardDateKey);
    var count = await StorageService.instance.getInt(_favoriteRewardCountKey) ?? 0;

    if (savedDate != today) {
      count = 0;
      await StorageService.instance.saveString(_favoriteRewardDateKey, today);
    }

    if (count >= IapConstants.maxFavoriteRewardsPerDay) return false;

    const amount = IapConstants.favoriteQuoteReward;
    _coins += amount;
    count++;
    await StorageService.instance.saveInt(_favoriteRewardCountKey, count);
    _emitCoinEarned(amount, 'favoriteRewardEarned');
    await _saveLocal();
    notifyListeners();
    return true;
  }
  ...
  Future<bool> rewardForShare() async {
    ...
    const amount = IapConstants.shareQuoteReward;
    _coins += amount;
    count++;
  ...
  Future<void> setQuoteFont(String font) async {
    if (font != 'outfit' && !hasPremiumFonts) return;
    _quoteFont = font;
    await _saveLocal();
    notifyListeners();
  }
  ...
  Future<void> setNotificationHour(int hour) async {
    if (!hasCustomNotification) return;
    _notificationHour = hour.clamp(0, 23);
    await _saveLocal();
    notifyListeners();
  }
